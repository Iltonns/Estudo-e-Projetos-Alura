...
CREATE TABLE IF NOT EXISTS `vendas_sucos`.`notas` (
  ...
  PRIMARY KEY (`NUMERO`),
  INDEX `FK_CLIEBTES` (`CPF` ASC) VISIBLE,
  INDEX `FK_VENDEDORES` (`MATRICULA` ASC) VISIBLE,
  CONSTRAINT `FK_CLIEBTES`
    FOREIGN KEY (`CPF`)
...
CREATE TABLE IF NOT EXISTS `vendas_sucos`.`produtos` (
  ...
  `PRECO_LISTA` VARCHAR(50) NULL DEFAULT NULL,
  PRIMARY KEY (`CODIGO`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;
...
CREATE TABLE IF NOT EXISTS `vendas_sucos`.`itens_notas` (
  `NUMERO` VARCHAR(5) NOT NULL,
  `CODIGO` VARCHAR(10) NOT NULL,
  `QUABTIDADE` INT NULL DEFAULT NULL,
